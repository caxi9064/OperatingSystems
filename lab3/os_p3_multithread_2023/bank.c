...
int main (int argc, const char * argv[] ) {
    ...
    for (int i = 0; i < num_atms; i++) {
        ...
        args[0] = i;
        args[1] = num_ops;
        if (pthread_create(&atm_threads[i], NULL, atm_thread, (void*) args)) {
    ...
    for (int i = 0; i < num_workers; i++) {
        ...
        args[0] = i;
        args[1] = num_ops;
        if (pthread_create(&worker_threads[i], NULL, worker_thread, (void*) args)) {
...
void* worker_thread(void* arg) { 
    ...
    while(!local){
        ...
        //pthread_cond_broadcast(&full);
        // if(bank_numop >= num_ops){ // no more operation in queue, exit
        //     pthread_mutex_unlock(&mutex);
        //     return NULL;
        // }
        while (queue_empty(circular_queue)) {
            //printf("1bank numop: %d num_ops: %d \n", bank_numop, num_ops);
            // if(bank_numop >= num_ops){ // no more operation in queue, exit
            //     //printf("2bank numop: %d num_ops: %d \n", bank_numop, num_ops);
            //     return NULL;
            // }
            if(bank_numop >= num_ops){ // no more operation in queue, exit
                pthread_mutex_unlock(&mutex);
        ...
        // if(bank_numop >= num_ops){ // no more operation in queue, exit
        //     printf("3bank numop: %d num_ops: %d \n", bank_numop, num_ops);
        //     return NULL;
        // }
        // printf("iteration: %d  \n", i);
        // i++;
